...
%---------------------------Linear Regression------------------------------
    ...
    %%% Ta parakatw plot einai idia me auta tou Erotimatos 5, gi'ayto 
    %%% ta exoume se comment.
%     figure(cfig);
%     scatter(x,y)
%     % ylim([0 2.5])
...
for i = 1:6
    ...
    %%% Parakatw ginontai ta plot poy mas voithane me thn epilogh krithriwn
    %%% gia to PCA, dhladh: Scree Plot kai Explained Variance Percentage
    %%% Ta vazoume se comment gia oikonomia 
...
%     cfig = cfig + 1;
    ...
    %%% Parakatw emfanizontai ta plot twn PCs
    %%%  Ta exoyme valei se comment, gia oikonomia diagrammatwn
%     if d == 2
%         figure(cfig);
...
%     end
    ...
    %%%Parakatw paroysiazontai epishs ta diagrammata meta to PCA
%     figure(cfig);
%     scatter(xinit,y)
...
%explained variance percentage plot poy xrhsimopoioyntai sthn PCA. Opote
...
%th meiwsh diastashs den einai eksisoy kalh se oles tis xwres. Gia
%paradeigma se xwres opws h Italia kai h Toyrkia fainetai na prosarmozetai
%arketa kala enw se xwres opws h Ellada kai h Elvetia oxi kai toso kala.
...
%oti provlepsh tha mporouse na ginei gia xronikes ysterhseis mexri kai 3
