...
death_table = readtable('Covid19Deaths.xlsx','basic',true);
confirmed_table = readtable('Covid19Confirmed.xlsx','basic',true);
% deaths = table2array(death_table(:,3:end));
% confirmed = table2array(confirmed_table(:,3:end));
...
n1 = ddays;
n2 = cdays;
%%%Se ayto to kommati kwdika metatrepontai oi arnitikes times twn dedomenwn se 0%%% 
...
%%%Edw lamvanetai h empeirikh pdf twn dedomenwn
pdfemp_deaths = (countrydeaths/sum(countrydeaths));
pdfemp_conf = (countryconf/sum(countryconf));
...
for i = 1:5
    ...
    switch i
        %%%Oi 5 prwtes einai aytes me ta kalytera apotelesmata
        case 1
...
%%%Se auto to kommati kwdika vriskoume ta kalytera MSE kai NRMSE
...
%NA DIKAIOLOGISOUME TO NAN H NA TO VGALOUME!!!!!!!!!!!!!!!!!!!!!!!!!!!
